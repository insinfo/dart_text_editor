...
ParagraphLayoutResult layout(String text, {double width = 100, double charW = 10}) {
  final node = ParagraphNode([TextRun(0, text, InlineAttributes())]);
  final layouter = ParagraphLayouter(MeasureCache(FakeTextMeasurer(charWidth: charW)));
...
void main() {
  ...
  final layouter = ParagraphLayouter(MeasureCache(FakeTextMeasurer(charWidth: 10)));
      final constraints = PageConstraints(width: 100, height: 1000);
      final res = layouter.layout(node, constraints);
      final totalLen = res.lines.expand((l) => l.spans).fold<int>(0, (s, sp) => s + (sp.endInNode - sp.startInNode));
      expect(totalLen, equals(3));
      final contemEspaco = res.lines.expand((l) => l.spans).any((sp) => node.text.substring(sp.startInNode, sp.endInNode).contains(' '));
    ...
    test('T4: quebra de linha preserva offsets', () {
      ...
      for (int l = 0; l < res.lines.length; l++) {
        ...
        for (final span in line.spans) {
          print('  span [${span.startInNode}, ${span.endInNode}): "${span.run.text}"');
        }
      }
      // linha 0: offsets 0,1,2; linha 1: offsets 3,4,5
      for (var i = 0; i <= text.length; i++) {
        int lineIdx = i < 3 ? 0 : 1;
        final xy = layouter.getCaretXY(res, lineIdx, i - (lineIdx == 1 ? 3 : 0));
        final j = layouter.getIndexFromXY(res, xy.dx, xy.dy);
