  ...
  group('Delete/Backspace Paragraph Merge Tests', () {
    ...
    test('Backspace at start of paragraph merges with previous', () {
      ...
      editor.state = editor.state
          .copyWith(selection: Selection.collapsed(const Position(1, 0)));

      editor.execute(BackspaceCommand());

      expect(editor.state.document.nodes.length, 2);
      expect(editor.state.document.nodes[0].getText(),
    ...
    test('Delete at end of paragraph merges with next', () {
      ...
      editor.state = editor.state.copyWith(
          selection: Selection.collapsed(
              const Position(0, 15)) // O tamanho de "First paragraph" é 15
          );
    ...
    test('Delete at end of last paragraph does nothing', () {
      ...
      editor.state = editor.state.copyWith(
          selection:
              Selection.collapsed(Position(lastParagraph, lastPosition)));

      final before =
          editor.state.document.copyWith(); // Use copyWith() em vez de clone()
      editor.execute(DeleteCommand());

      expect(editor.state.document.nodes.length, before.nodes.length);
      expect(editor.state.document.nodes.last.getText(),
          before.nodes.last.getText());
    });

    test('Backspace at start of first paragraph does nothing', () {
      editor.state = editor.state
          .copyWith(selection: Selection.collapsed(const Position(0, 0)));
      ...
      expect(editor.state.document.nodes.length, before.nodes.length);
      expect(editor.state.document.nodes.first.getText(),
    ...
    test('Undo restores merged paragraphs correctly', () {
      ...
      expect(editor.state.document.nodes[0].getText(), 'First paragraph');
      expect(editor.state.document.nodes[1].getText(), 'Second paragraph');
    });
    ...
    test('Backspace operation can be batched for a single undo', () async {
      ...
      editor.state = editor.state.copyWith(
          selection:
              Selection.collapsed(const Position(1, 1)) // 1 caractere dentro
          );
      ...
      await Future.delayed(
          const Duration(milliseconds: 250)); // Espera o batch finalizar
